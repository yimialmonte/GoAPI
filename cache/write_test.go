...
var (
	_ http.ResponseWriter = (*mockWriter)(nil)
)
...
func TestWriter(t *testing.T) {
	...
	if h2.Get("test") != "value" {
		t.Error("Value not store in the header")
	}
	...
	if w.response.code != c {
		t.Error("Status code not writter")
	}
	h2 = mw.header
	if h2.Get("test") != "value" {
	...
	n, err := w.Write(bd)
	if err != nil {
		t.Fatalf("Unexpected error while writting: %s", err)
	}
	if n != len(bd) {
